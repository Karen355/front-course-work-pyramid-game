...
let startTime;
let endTime;
let totalGameTime; // итоговое время 1 игры

// получаем доступ на странице к разделам с очками, правилами, именем пользователя и результатами
...
function startGame() {
  ...
  if (scoreElement) {
    ...
    // saveScoreToStorage(stack.length - 2);
  }
...
function saveScoreToStorage(score) {
  if (username.value) {
    ...
    localStorage.setItem(username.value, JSON.stringify(gameData));
    totalGameTime = "";
  }
}
...
function calculateGameTime() {
  // проверяем, были ли установлены начальное и конечное время
  if (startTime && endTime) {
    ...
    // округляем время до ближайшей секунды
    const roundedGameTimeInSeconds = Math.round(gameTimeInSeconds);
    // выводим время игры
    console.log(`Время игры: ${roundedGameTimeInSeconds} секунд`);
    totalGameTime = `${roundedGameTimeInSeconds} ${
      roundedGameTimeInSeconds % 10 == 1 && roundedGameTimeInSeconds % 100 != 11
        ? "секунда"
        : roundedGameTimeInSeconds % 10 >= 2 &&
          roundedGameTimeInSeconds % 10 <= 4 &&
          (roundedGameTimeInSeconds % 100 < 10 ||
            roundedGameTimeInSeconds % 100 >= 20)
        ? "секунды"
        : "секунд"
    }`;
  }
